...
The following is an example `rc.xml` (which would be placed in
`~/.config/openbox/rc.xml`) for people who use `focus-follows-mouse`.
The default configuration is geared towards a `click-to-focus` model.
If you use `focus-follows-mouse`, this example may be helpful fo setting up
your bindings, or you could use it as a template and edit it to your liking.

Note that it removes the `Raise` action when clicking in the Client context,
and moves the `Raise` action from the Alt-Click Press to Click context,
so that windows can be moved without being raised.
...
<openbox_config ns="http://openbox.org/3.4/rc">
